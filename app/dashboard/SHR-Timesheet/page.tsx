 ...
 const firstAllowedMonday = useMemo(() => {
  ...
  const handleDateChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setSelectedSubmitDate(null);
      ...
      useEffect(() => {
        if (role === 'SUPER_HR') return;   // ✅ stop here
        const fetchDOJ = async () => {
          try {
            ...
            if (employee.dateOfJoining) {
              setJoiningDate(dayjs(employee.dateOfJoining));
            }
      ...
      }, [role]);   // ✅ include role dependency
  

  const fetchHolidaysForWeek = useCallback(async () => {
    try {
      // ✅ HARD GUARD — SUPER_HR must have employee
      if (role === 'SUPER_HR' && !selectedEmployeeId) {
        setHolidayMap({});
        return;
      }
      ...
      const results = await Promise.all(
        weekYears.map(year =>
          role === 'SUPER_HR'
            ? holidayService.getAllHolidays(year, selectedEmployeeId || undefined)
  ...
  }, [role, selectedEmployeeId, weekYears]);
      ...
      let results;
        ...
        if (role === 'SUPER_HR') {
          if (!selectedEmployeeId) {
            // stop early — nothing to fetch yet
            setLeaveMap({});
            return;
          }
          ...
          results = await Promise.all(
            weekYears.map(year =>
              leaveService.getApprovedLeaves(year.toString())
            )
          );
        }
  ...
  const fetchData = useCallback(async () => {
    ...
    // ✅ ROLE SWITCH — HARD GATE
    if (role === 'SUPER_HR') {
      if (!selectedEmployeeId) {
        setRows([]);
        setWeekStatus('DRAFTED');
        setManagerComment(null);
        return; // stop — nothing to load yet
      }

      response = await managerTimeSheetService.getEmployeeTimesheets(
        selectedEmployeeId,
        startDate,
        endDate
      );
...
useEffect(() => {
  ...
  load();
}, [role,selectedEmployeeId,fetchData, fetchLeavesForWeek, fetchHolidaysForWeek]);
  ...
  const saveAll = async () => {
    ...
    try {
      ...
      if (toCreate.length > 0) {
        ...
        const createTimesheetsByRole = async () => {
          if (role === 'SUPER_HR') {
            if (!selectedEmployeeId) {
              throw new Error('Employee must be selected');
            }
            console.log('Creating timesheets as SUPER_HR for employee:', selectedEmployeeId);
            return timesheetService.createTimesheetsAsSuperHr(
              selectedEmployeeId,
              toCreate
            );
          }
        
          return timesheetService.createTimesheets(toCreate);
        };
        
        const res = await createTimesheetsByRole();
                    ...
                    try {
                      ...
                      if (ids.length > 0) {
                        if (role === 'SUPER_HR') {
                          ...
                          console.log('Deleting timesheets as SUPER_HR for employee:', selectedEmployeeId);
                          await timesheetService.deleteTimesheetsAsSuperHr(
                            selectedEmployeeId,
                            ids
                          );
                          await fetchData();
