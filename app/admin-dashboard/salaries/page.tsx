...
import { ExtraDecisionRequestDTO, ExtraDecisionType } from "@/lib/api/types";
...
export default function AdminPage() {
  ...
  const [pendingEmployees, setPendingEmployees] = useState<{ employeeId: string; employeeName: string; companyId?: string; workedHours: number }[]>([]);
  const [extraWorkEmployees, setExtraWorkEmployees] = useState<{
    employeeId: string;
    employeeName: string;
  ...
  const fetchExtraWorkEmployees = async () => {
    ...
    try {
      ...
      Swal.fire({
        icon: "error",
        title: "Failed to fetch extra work employees",
      });
    }
  };
  ...
  const downloadAsPDF = (list: typeof pendingEmployees) => {
    const doc = new jsPDF();
    ...
    <div className="container mx-auto p-4 md:p-6 space-y-10">
      ...
      <Tabs defaultValue="generate" className="w-full">
        ...
        <TabsList className="grid grid-cols-3 w-full">
          ...
          <TabsTrigger value="extraEmployees">Extra Work Employees</TabsTrigger>
        </TabsList>
                          ...
                          <CardContent className="space-y-6">
                              ...
                              onClick={async () => {
                                ...
                                try {
                                  ...
                                  if (result.isConfirmed) {
                                    setPendingEmployees(res.response?.pendingValidationEmployees || []);
                                    setExtraWorkEmployees(res.response?.extraWorkEmployees || []);
                                  }
                                } catch (err) {
                                  console.error(err);
                            ...
                            <Tabs defaultValue="pending" className="mt-6 w-full">
                              ...
                              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4">
                                ...
                                {/* Download Button – right-aligned on desktop */}
                                              <Button
                                                  variant="outline"
                                                  size="sm"
                                                  onClick={() => downloadAsPDF(pendingEmployees)}
                                    ...
                                    <tbody className="divide-y divide-gray-200 bg-white">
                                      {pendingEmployees.map((emp) => (
                                        <tr key={emp.employeeId}>
                                          ...
                                          <td className="px-6 py-4">{emp.companyId ?? "—"}</td>
                                          <td className="px-6 py-4">{emp.workedHours}</td>
                                        </tr>
                                      ))}
                                    </tbody>
                              ...
                              <TabsContent value="extra" className="w-full">
                                  ...
                                  {extraWorkEmployees.map((emp) => (

                                  <tr key={emp.employeeId}>

                                  <td className="px-6 py-4">{emp.employeeName}</td>

                                  <td className="px-6 py-4">{emp.extraWorkedDays}</td>
                      ...
                      {/* ───────────────────────────────
                          TAB 3 → Extra Work Employees
                      ─────────────────────────────── */}

                      <TabsContent value="extraEmployees">
                        <Card>

                          <CardHeader>
                            <CardTitle>Extra Work Employees</CardTitle>
                          </CardHeader>
