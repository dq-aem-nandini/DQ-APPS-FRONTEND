...
export default function addHoliday() {
  ...
  const [editingHoliday, setEditingHoliday] = useState<HolidaysDTO | null>(
    null
  );
  ...
  useEffect(() => {
    ...
    async function loadEmployees() {
      try {
        const res = await manualInvoiceService.getEmployeesByClientId(
          clientId,
          false
        );
        const data: ClientEmployeeMinResponseDTO | null = Array.isArray(
          res.response
        )
          ? res.response[0]
          : res.response ?? null;
  ...
  return (
      ...
      <div className="bg-white/80 backdrop-blur-lg rounded-2xl shadow-xl p-8 mb-8">
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
          ...
          <div className="relative inline-block group">
            <div
              className={`${selectedEmployeeIds.length === 0 ? "cursor-not-allowed" : ""
                }`}
            >
              {/* Right Button */}
              <Button
                ...
                onClick={async () => {
                  ...
                  if (selectedHolidays.length > 0) {
                    ...
                    try {
                      ...
                      const selectedHolidayObjects =
                        getSelectedHolidayObjects();
    ...
    `}
      ...
      <div className="bg-white rounded-2xl shadow-lg p-6">
        {loading && <p className="text-gray-500">Loading holidays...</p>}
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500">{holiday.holidayDate}</p>
                </div>
              </div>
            </label>
          ))}
