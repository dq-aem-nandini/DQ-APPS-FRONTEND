    ...
    try {
      ...
      if (res.flag) {
        ...
        if (employee?.employeeId) {
          const statusRes = await employeePunchService.getPunchStatus(
            employee.employeeId
          );
          if (statusRes.flag) {
          ...
          <div className="lg:col-span-2">
            <Card className="h-full border-0 shadow-2xl bg-white/80 backdrop-blur-xl rounded-3xl">
              <CardContent className="p-5 sm:p-6 md:p-10">
                <div className="grid grid-cols-1 grid-cols-1 md:grid-cols-[minmax(0,1fr)_200px] lg:grid-cols-[minmax(0,1fr)_240px] gap-6 md:gap-8 items-center">
                  {" "}
                  {/* LEFT SIDE – Time + Date + Status */}
                  <div className="flex flex-col items-center md:items-start text-center md:text-left ">
                    <div className="text-xl sm:text-2xl md:text-3xl lg:text-4xl xl:text-5xl font-bold text-gray-900 mb-2 leading-tight truncate">
                    ...
                    <span
                      className={`mt-2 inline-flex items-center justify-center px-4 sm:px-5 md:px-6 py-1.5 sm:py-2 text-xs sm:text-sm md:text-base font-semibold rounded-full whitespace-nowrap
                  ...
                  <div className="flex justify-center md:justify-end mt-4 md:mt-0">
                    <Button
                      ...
                      className={`w-full sm:w-auto md:w-56 py-3 sm:py-4 text-sm sm:text-base md:text-lg font-semibold rounded-2xl transition-all duration-300 shadow-lg ${attendanceStatus?.nextAction === "OUT"
                ...
                {(attendanceStatus?.firstClockIn ||
                  attendanceStatus?.lastClockOut) && (
                    <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-gray-600 justify-items-center md:justify-items-stretch">
                      {" "}
                      {attendanceStatus?.firstClockIn && (
