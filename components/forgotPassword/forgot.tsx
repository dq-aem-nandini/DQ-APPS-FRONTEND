...
const ForgotPassword: React.FC = () => {
  const [step, setStep] = useState<"identifier" | "otp" | "newPassword">(
    "identifier"
  );
  ...
  const sendOTP = async (id: string) => {
    ...
    try {
      ...
      if (res.response?.status === "OTP_SENT") {
        setError(""); // clear old error
        setSuccessMessage(res.response.message);
        setCanEnterOTP(true);
        setStep("otp");

        if (res.response.expiry) {
          const expiryMillis = new Date(res.response.expiry).getTime();
          setExpiryTime(expiryMillis);

          // immediately calculate remaining time (prevents 1s resend flash)
          const remaining = Math.max(
            0,
            Math.floor((expiryMillis - Date.now()) / 1000)
          );
          setTimeLeft(remaining);
        }
      } else {
        setError(res.response?.message || res.message);
  ...
  useEffect(() => {
    ...
    updateTimer(); // run immediately (important)
  ...
  const handleResendOTP = async () => {
    // clear everything related to previous OTP
    setError("");
    setSuccessMessage("");
    setOtpDigits(["", "", "", "", "", ""]);
    setExpiryTime(null);
  ...
  const handleVerifyOTP = async (e: React.FormEvent) => {
    ...
    try {
      const res = await passwordService.verifyOTP(identifier, fullOtp);
      ...
      if (res.response?.verified) {
        ...
        setTimeout(() => {
          ...
          // 🔥 CLEAR ONLY THE VERIFY MESSAGE AFTER PAGE CHANGE
          setTimeout(() => {
            setSuccessMessage("");
          }, 50);
          ...
          <form onSubmit={handleVerifyOTP} className="space-y-6">
            {canEnterOTP && (
              <>
                ...
                OTP expires in <span className="font-semibold text-gray-800">{formatTimer(timeLeft)}</span>
              </span>
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  className={`w-full rounded-lg border pr-12 py-3 px-4 transition-all ${
                    passwordCheckStatus === "used" || newPasswordError
